        ...
        while(i != MAX_PT_SIZE){
            if(pd[i].pd_pres == 0){
                if((initial_pde_counter == 0) || ((i + initial_pde_counter) + (frames > 0) < MAX_PT_SIZE)){
                    ...
                    mask = disable();
                    counter = 0;
                    pde_counter = initial_pde_counter + (frames > 0);
                    for(base_addr = PAGE_DIR_ADDR_START; base_addr < PD_PT_AREA_END; base_addr += PAGE_SIZE){
                        counter++; // keep track of when base_addr should start from
                        addr = (char *)base_addr;
                        pt = (pt_t *) addr;
                        if(pt[0].pt_avail == 0){
                            pde_counter--;
                    ...
                    if(pde_counter == 0){
                        unsigned int end = PAGE_DIR_ADDR_START + (PAGE_SIZE * counter);
                        unsigned int offset = (PAGE_SIZE * (initial_pde_counter + (frames > 0)));
                        for(base_addr = end - offset; base_addr < end; base_addr += PAGE_SIZE){
                            addr = (char *)base_addr;
                            pt = (pt_t *) addr;
                            ...
                            addr = (char *)base_addr;
                            pt = (pt_t *) addr;
                            if((frames > 0) && (base_addr + PAGE_SIZE == end)){
                                for(j = 0; j < frames; j++){
                                    pt[j].pt_avail = 3;
                                }
                            }else{
                ...
                if(initial_pde_counter == 0 || (initial_pde_counter == 1 && frames == 0)){
                    ...
                    while(j < initial_pde_counter + (frames > 0)){
                        ...
                        if(counter > 0 || (counter == (frames + initial_pde_counter * MAX_PT_SIZE))){
                            ...
                            j = initial_pde_counter + (frames > 0);
                        }
                    }
                    if(counter == (frames + initial_pde_counter * MAX_PT_SIZE)){
